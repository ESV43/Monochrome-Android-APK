...
export async function initializeSettings(scrobbler, player, api, ui) {
    ...
    // YTM Connect button
    const connectYtmBtn = document.getElementById('connect-ytm-btn');
    if (connectYtmBtn) {
        const updateYtmBtn = () => {
            const creds = ytmSettings.getCredentials();
            if (creds.cookie) {
                connectYtmBtn.textContent = 'Connected (Click to reconnect)';
                connectYtmBtn.classList.add('success');
            } else {
                connectYtmBtn.textContent = 'Connect';
                connectYtmBtn.classList.remove('success');
            }
        };
        updateYtmBtn();

        connectYtmBtn.addEventListener('click', () => {
            if (window.startYTMLogin) {
                window.startYTMLogin();
            } else {
                alert('YTM Login is only available in the Android app.');
            }
        });

        window.addEventListener('ytm-login-success', (e) => {
            const { cookie, authUser } = e.detail;
            ytmSettings.setCredentials(cookie, authUser);
            updateYtmBtn();
            alert('YouTube Music connected successfully!');
        });
    }
