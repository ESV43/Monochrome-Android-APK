...
export const ytmSettings = {
    CREDENTIALS_KEY: 'ytm-credentials',

    getCredentials() {
        try {
            const stored = localStorage.getItem(this.CREDENTIALS_KEY);
            return stored ? JSON.parse(stored) : { cookie: '', authUser: '0' };
        } catch {
            return { cookie: '', authUser: '0' };
        }
    },

    setCredentials(cookie, authUser = '0') {
        localStorage.setItem(this.CREDENTIALS_KEY, JSON.stringify({ cookie, authUser }));
    },
