...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (spotifyBtn && appleBtn && ytmBtn) {
        ...
        // YTM Fetch Playlists Logic
        const fetchPlaylistsBtn = document.getElementById('ytm-fetch-playlists-btn');
        const playlistsSelect = document.getElementById('ytm-playlists-select');
        const ytmUrlInput = document.getElementById('ytm-url-input');

        fetchPlaylistsBtn?.addEventListener('click', async () => {
            const originalText = fetchPlaylistsBtn.textContent;
            fetchPlaylistsBtn.textContent = 'Loading...';
            fetchPlaylistsBtn.disabled = true;

            try {
                const playlists = await MusicAPI.instance.ytmAPI.getUserPlaylists();
                if (playlists.length > 0) {
                    playlistsSelect.innerHTML = '<option value="">Select a playlist...</option>' + 
                        playlists.map(p => `<option value="https://music.youtube.com/playlist?list=${p.id.slice(2)}">${p.title}</option>`).join('');
                    playlistsSelect.style.display = 'block';
                    
                    playlistsSelect.onchange = (e) => {
                        ytmUrlInput.value = e.target.value;
                    };
                } else {
                    alert('No playlists found. Make sure you are connected to YouTube Music in Settings.');
                }
            } catch (err) {
                console.error('Failed to fetch YTM playlists:', err);
                alert('Failed to fetch playlists. Please check your connection in Settings.');
            } finally {
                fetchPlaylistsBtn.textContent = originalText;
